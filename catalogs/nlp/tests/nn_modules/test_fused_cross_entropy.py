"""Tests for FusedLinearCELoss module."""
...
TEST_ARTIFACTS_DIR = Path(__file__).parent.parent / "test_artifacts" / "nn_modules"


@pytest.fixture(scope="session", autouse=True)
def clear_test_error_maps():
    """Clear test artifacts directory before running tests."""
...
class TestTorchLinearCELoss:
    """Test suite for TorchLinearCELoss (baseline implementation)."""
    ...
    def test_weight_tying(self, D, V, device, dtype):
        ...
        # Verify the shared weight received gradients
...
class TestFusedLinearCELoss:
    """Test suite for FusedLinearCELoss (optimized implementation)."""
    ...
    def test_matches_torch_implementation(self, D, V, device, dtype):
        """Test that FusedLinearCELoss matches TorchLinearCELoss numerically."""
        if device != 'cuda':
            pytest.skip("FusedLinearCELoss requires CUDA")
        ...
        # Relaxed tolerances for fused implementations
    ...
    @pytest.mark.parametrize("D,V", [(128, 1000)])
    def test_weight_tying(self, D, V, device, dtype):
        """Test weight tying functionality with fused implementation."""
        if device != 'cuda':
            pytest.skip("FusedLinearCELoss requires CUDA")
...
class TestFusedVsTorch:
    """Cross-validation tests comparing fused and torch implementations."""
    ...
    def test_gradient_equivalence(self, D, V, use_weight_tying, device, dtype):
        ...
        # Relaxed tolerances for fused kernel
        tols = get_tolerances_for_dtype(dtype)
        if dtype in [torch.float16, torch.bfloat16]:
            tols = {'atol': 1e-2, 'rtol': 1e-1}
        ...
        # Check weight gradient equivalence
        if use_weight_tying:
