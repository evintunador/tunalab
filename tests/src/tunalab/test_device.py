...
def test_get_default_device_cuda():
...
@pytest.mark.skipif(
    not (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()) or torch.cuda.is_available(),
    reason="MPS not available or CUDA takes priority"
)
def test_get_default_device_mps():
...
@pytest.mark.skipif(
    ...
    reason="Accelerators available, CPU fallback not applicable"
)
def test_get_default_device_cpu_fallback():
...
@pytest.mark.skipif(
    torch.cuda.is_available() or (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()),
    reason="Accelerators available, can't test CPU-only scenario"
)
...
@pytest.mark.skipif(not torch.cuda.is_available() or torch.cuda.device_count() < 2, reason="Requires multiple CUDA devices")
def test_get_available_devices_multiple_cuda():
...
@pytest.mark.skipif(
    ...
    reason="Requires accelerators to test CPU exclusion"
)
def test_get_available_devices_cpu_not_included_with_accelerators():
    """Test that CPU is not included when accelerators are available."""
    devices, devices_with_ranks = get_available_devices()
    
    # CPU should not be in the list when accelerators are available
